...
function showPanel(id) {
	...
	$
			.ajax({
				type : "POST", // 提交方式
				url : "../lookCombo.action", // 路径
				data : {
					comboId : id
				},
				dataType : "json",
				success : function(respData) {
					var itemsDtoList = respData.items
					...
					if (itemsDtoList.length > 0) {
						...
						for (var i = 0; i < itemsDtoList.length; i++) {
							...
							for (var j = 0; j < items.itemList.length; j++) { // 细项集合的循环
								var tr = $("<tr></tr>");
								if (j == 0) {
									var seq = $("<td style=\"width: 6.5%;\"></td>"); // 序号
									seq.text(i + 1);
									seq.attr("rowspan", items.itemList.length);
									var itemsName = $("<td style=\"width: 38%;\"></td>"); // 项目名
					...
					$("#price").val(respData.combo.comboPrice);
...
function calculatePrice(checkbox) {
	var price = $("#price").val();
	if (checkbox.checked) {
		totalPrice = parseInt($(checkbox).attr("itemsPrice"))
				+ parseInt(totalPrice);
		$("#price").val(totalPrice);
	} else {
		totalPrice = parseInt(totalPrice)
				- parseInt($(checkbox).attr("itemsPrice"));
		$("#price").val(totalPrice);
	}
}
