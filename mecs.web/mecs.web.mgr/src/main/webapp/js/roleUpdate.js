...
function showUpdatePanel() {
	$
			.ajax({
				url : "../showRoleUpdate.action",
				type : "post",

				dataType : "json",
				success : function(respData) {
					var list = respData.allMenu;
					// 回填角色名
					$("#role").val(respData.role.roleName);
					$("#fid").val(respData.role.roleId);
					var menu = $("#menu");
					if (list.length != 0) {
						for (var i = 0; i < list.length; i++) {
							if (list[i].menuPid == 0) {
								...
								for (var a = 0; a < respData.roleMenu.length; a++) {
									if (respData.roleMenu[a] == list[i].menuId) {
								...
								for (var j = 0; j < list.length; j++) {
									if (list[j].menuPid == list[i].menuId) {
										...
										checkbox2.attr("name", "check"
												+ (i + 1));
										checkbox2.attr("id", 2);
										checkbox2.attr("value", list[j].menuId);
										for (var b = 0; b < respData.roleMenu.length; b++) {
											if (respData.roleMenu[b] == list[j].menuId) {
