window.onload = function() {
	$.ajax({
		type: "post",
		url: "../userSession.action",
		data: {},
		dataType: "json",
		success: function(respdata) { //res为相应体,function为回调函数
			console.log(respdata);
			var user=respdata.user;
			lockword=user.aiPassword;
			console.log(lockword);
			$(".adminName").html(user.aiName);
			$(".userName").html(user.aiName);
		},
		error: function() {
			layer.alert('禁用失败，网络异常', {
				icon: 5
			});
		}
	});
   ...
   //公告层
    function showNotice(){
        layer.open({
            ...
            btn: ['火速围观'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">体检系统全面升级啦。</p></pclass></p><p><span class="layui-red">请工作人员严谨工作，管理员请认真校对后再对系统做出更改！</span></p></div>',
    ...
    $("body").on("click","#unlock",function(){
        if($(this).siblings(".admin-header-lock-input").val() == ''){
            ...
            if($(this).siblings(".admin-header-lock-input").val() == lockword){
